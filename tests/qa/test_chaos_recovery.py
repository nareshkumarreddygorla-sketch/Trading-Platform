...
async def test_broker_latency_spike_timeout_releases_reservation():
    ...
    req = OrderEntryRequest(signal=sig, quantity=10, order_type=OrderType.LIMIT, limit_price=100.0, idempotency_key="qa_latency_001", source="qa")
...
async def test_redis_down_idempotency_rejects_no_broker_call():
    ...
    req = OrderEntryRequest(
        signal=Signal(strategy_id="s1", symbol="R", exchange=Exchange.NSE, side=SignalSide.BUY, score=0.9, portfolio_weight=0.1, price=100.0),
        quantity=10,
