...
class _OrderRecord:
    ...
    cancel_time: Optional[float] = None  # monotonic time of cancellation
...
class SurveillanceEngine:
    ...
    def __init__(
        self,
        thresholds: Optional[SurveillanceThresholds] = None,
    ...
    def _detect_layering(
        ...
        recent_orders = [
            ...
            if o.algo_id == algo_id and o.timestamp >= cutoff
        ]
        ...
        for side in ("BUY", "SELL"):
            ...
            if cancel_ratio >= th.layering_cancel_ratio:
                ...
                severity = (
                    AlertSeverity.CRITICAL if cancel_ratio > 0.9
                    else AlertSeverity.HIGH if cancel_ratio > 0.8
    ...
    def _detect_spoofing(
        ...
        cancel_elapsed = (
            (cancelled_order.cancel_time or now_mono) - cancelled_order.timestamp
        )
        ...
        recent_orders = [
            o for o in self._orders[symbol]
            if o.order_id != cancelled_order.order_id
            and o.timestamp >= (now_mono - 3600)  # last hour
        ]
        ...
        severity = (
            AlertSeverity.CRITICAL if cancel_elapsed < 1.0 and size_ratio > 10
            else AlertSeverity.HIGH if cancel_elapsed < 2.0
            else AlertSeverity.MEDIUM
        )
    ...
    def _detect_wash_trade(
        ...
        algo_recent = [
            t for t in self._algo_trades.get(new_trade.algo_id, deque())
            if (
        ...
        for matching in algo_recent:
            ...
            severity = (
                AlertSeverity.CRITICAL if time_between < 5
                else AlertSeverity.HIGH if time_between < 30
                else AlertSeverity.MEDIUM
            )
