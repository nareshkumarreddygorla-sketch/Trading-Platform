            ...
            if not db_url:
                if env == "production":
                    raise RuntimeError(
                        ...
                        "Set ENV=development for SQLite."
                    )
...
def check_database_health(engine: Optional[Engine] = None, max_size_gb: float = 80.0) -> dict:
                ...
                with eng.connect() as conn:
                    ...
                    if row:
                        ...
                        if result["size_pct"] > 90:
                            ...
                            result["error"] = f"Database disk usage critical: {result['size_gb']:.1f}GB ({result['size_pct']:.0f}% of {max_size_gb}GB limit)"
                            logger.critical(result["error"])
                        elif result["size_pct"] > 75:
                            logger.warning("Database disk usage high: %.1fGB (%.0f%% of limit)", result["size_gb"], result["size_pct"])
