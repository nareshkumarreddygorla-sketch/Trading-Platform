...
class TradeOutcomeRepository:
    ...
    def record_outcome(
        ...
        regime_at_entry: str = "",
        features_at_entry: dict[str, float] | None = None,
        ...
        with session_scope() as session:
            ...
            session.add(outcome)
            session.commit()
            logger.info("Recorded trade outcome: %s %s PnL=%.2f (%.2f%%)", side, symbol, realized_pnl, pnl_pct * 100)
        ...
        with session_scope() as session:
            rows = (
                session.query(
                    ...
                    func.sum(func.cast(TradeOutcomeModel.realized_pnl > 0, Integer)).label("wins"),
    ...
    def get_recent_outcomes(self, limit: int = 100) -> list[dict[str, Any]]:
        ...
        with session_scope() as session:
            rows = session.query(TradeOutcomeModel).order_by(TradeOutcomeModel.exit_time.desc()).limit(limit).all()
            return [self._row_to_dict(r) for r in rows]
