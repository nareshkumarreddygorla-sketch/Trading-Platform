...
class OpenTradeRepository:
    ...
    def upsert_trade(
        ...
        try:
            with session_scope() as sess:
                existing = sess.query(OpenTradeModel).filter(OpenTradeModel.trade_key == trade_key).first()
    ...
    def delete_trade(self, trade_key: str) -> bool:
        ...
        try:
            with session_scope() as sess:
                sess.query(OpenTradeModel).filter(OpenTradeModel.trade_key == trade_key).delete()
            return True
        except Exception as e:
            ...
            with session_scope() as sess:
                ...
                for r in rows:
                    if r.entry_price is None or r.entry_price <= 0:
                        logger.warning(
                            "Skipping corrupt open_trade row: trade_key=%s entry_price=%s", r.trade_key, r.entry_price
                        )
                        continue
                    trades.append(
                        {
                ...
                logger.info("Loaded %d open trades from DB (skipped %d corrupt)", len(trades), len(rows) - len(trades))
                return trades
        except Exception as e:
