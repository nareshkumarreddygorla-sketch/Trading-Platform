...
INTRADAY_ALPHA_SPECS: List[FeatureSpec] = [
    ...
    FeatureSpec("poc_deviation_bps", "float", "v1", "Price deviation from session POC (volume profile)", FeatureGroup.INTRADAY_ALPHA),
    FeatureSpec("value_area_upper_dev_bps", "float", "v1", "Deviation from value area upper (70%)", FeatureGroup.INTRADAY_ALPHA),
    FeatureSpec("value_area_lower_dev_bps", "float", "v1", "Deviation from value area lower (70%)", FeatureGroup.INTRADAY_ALPHA),
    FeatureSpec("minute_of_day_sin", "float", "v1", "Intraday seasonality sin(minute/390)", FeatureGroup.INTRADAY_ALPHA),
    FeatureSpec("minute_of_day_cos", "float", "v1", "Intraday seasonality cos(minute/390)", FeatureGroup.INTRADAY_ALPHA),
    FeatureSpec("vwap_dev_curvature", "float", "v1", "Second derivative of (p - VWAP) w.r.t. time", FeatureGroup.INTRADAY_ALPHA),
    FeatureSpec("liquidity_vacuum", "float", "v1", "Spread > k*median(spread) or vol < k*median(vol)", FeatureGroup.INTRADAY_ALPHA),
    FeatureSpec("vol_of_vol_20", "float", "v1", "Std of rolling 5-bar vol over 20 bars", FeatureGroup.INTRADAY_ALPHA),
    FeatureSpec("microstructure_noise_resid", "float", "v1", "Residual from rolling MA on mid (noise filter)", FeatureGroup.INTRADAY_ALPHA),
    FeatureSpec("spread_widening_signal", "float", "v1", "Predicted Δ spread from lagged spread/vol/OFI", FeatureGroup.INTRADAY_ALPHA),
    FeatureSpec("cross_stock_dispersion", "float", "v1", "Std of returns across sector constituents", FeatureGroup.INTRADAY_ALPHA),
    FeatureSpec("sector_momentum_rank", "float", "v1", "Rank of sector return over last N bars", FeatureGroup.INTRADAY_ALPHA),
]
