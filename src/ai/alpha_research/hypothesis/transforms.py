...
PRICE_TEMPLATES = [
    ...
    TransformTemplate(
        "momentum_multi_horizon", TransformFamily.PRICE, "price / price[t-h] - 1", {"horizon": [5, 10, 20]}, 3
    ),
    TransformTemplate(
        "mean_reversion_zscore", TransformFamily.PRICE, "zscore(price, window)", {"window": [10, 20, 40]}, 3
    ),
...
MICRO_TEMPLATES = [
    TransformTemplate(
        "ofi_variant",
        TransformFamily.MICROSTRUCTURE,
    ...
    TransformTemplate(
        "volume_cluster", TransformFamily.MICROSTRUCTURE, "volume / rolling_median(volume)", {"window": [20]}, 1
    ),
    TransformTemplate(
        "quote_imbalance_decay",
        TransformFamily.MICROSTRUCTURE,
        "quote_imbalance * exp(-decay*t)",
        {"decay": [0.1, 0.2]},
        2,
    ),
]
...
CROSS_TEMPLATES = [
    TransformTemplate(
        ...
        TransformFamily.CROSS_SECTIONAL,
        "symbol_return - sector_return",
        {"window": [5, 20]},
        2,
    ),
    TransformTemplate(
        "dispersion_divergence", TransformFamily.CROSS_SECTIONAL, "std(cross_sectional_returns)", {"window": [20]}, 1
    ),
...
REGIME_TEMPLATES = [
    TransformTemplate(
        "feature_high_vol",
        TransformFamily.REGIME_CONDITIONED,
        "feature * I(vol_regime=high)",
        {"feature_ref": ["momentum", "zscore"]},
        2,
    ),
    TransformTemplate(
        "feature_low_liq", TransformFamily.REGIME_CONDITIONED, "feature * I(liquidity=low)", {"feature_ref": ["ofi"]}, 1
    ),
]
...
NONLINEAR_TEMPLATES = [
    ...
    TransformTemplate(
        "percentile_transform", TransformFamily.NONLINEAR, "percentile_rank(feature, window)", {"window": [20, 40]}, 2
    ),
    TransformTemplate(
        "cross_product", TransformFamily.NONLINEAR, "feature_a * feature_b", {"pair": ["mom_vol", "ofi_zscore"]}, 2
    ),
]


def transform_templates() -> list[TransformTemplate]:
    return PRICE_TEMPLATES + MICRO_TEMPLATES + CROSS_TEMPLATES + REGIME_TEMPLATES + NONLINEAR_TEMPLATES
