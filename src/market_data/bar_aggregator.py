...
class TickToBarAggregator:
    ...
    def __init__(self, bar_cache, interval_seconds: int = 60):
        ...
        self._current: dict = defaultdict(
            lambda: {"open": None, "high": None, "low": None, "close": None, "volume": 0.0, "bucket_ts": None}
        )
    ...
    def push_tick(self, tick: Tick) -> Bar | None:
        """
        ...
        if bucket > cur["bucket_ts"]:
            bar = Bar(
                ...
                if isinstance(tick.exchange, Exchange)
                else Exchange(tick.exchange)
                if isinstance(tick.exchange, str)
                else Exchange.NSE,
                interval="1m",
                open=cur["open"],
                high=cur["high"],
                ...
                close=cur["close"],
                volume=cur["volume"],
                ts=datetime.fromtimestamp(cur["bucket_ts"], tz=UTC),
                source="aggregator",
            )
