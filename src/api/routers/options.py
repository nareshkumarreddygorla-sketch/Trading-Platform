...
_SPOT_DEFAULTS = {
    "NIFTY": 22500, "BANKNIFTY": 48000, "RELIANCE": 2500,
    "TCS": 3800, "INFY": 1500, "HDFCBANK": 1600,
    "SBIN": 750, "TATAMOTORS": 900, "ITC": 450,
}
_LOT_SIZES = {
    "NIFTY": 25, "BANKNIFTY": 15, "RELIANCE": 250,
    "TCS": 150, "INFY": 300, "HDFCBANK": 550,
    "SBIN": 1500, "TATAMOTORS": 1400, "ITC": 1600,
}
...
async def get_options_chain(
    ...
    def _contract(c):
        g = c.greeks
        return {
            "symbol": c.symbol, "strike": c.strike, "last_price": c.last_price,
    ...
    return {
        "underlying": chain.underlying, "spot_price": chain.spot_price,
        "expiry": chain.expiry, "days_to_expiry": days, "lot_size": lot_size,
...
async def calculate_greeks(req: GreeksRequest):
    ...
    return {
        "option_type": req.option_type, "spot": req.spot, "strike": req.strike,
        "days_to_expiry": req.days_to_expiry, "volatility": req.volatility,
        "price": round(r.price, 2), "delta": round(r.delta, 4),
        "gamma": round(r.gamma, 6), "theta": round(r.theta, 4),
        "vega": round(r.vega, 4), "rho": round(r.rho, 4),
    }
...
async def get_iv_surface(
    ...
    for days in expiries_days:
        ...
        for strike in strikes:
            ...
            surface.append({
                "days_to_expiry": days, "strike": strike,
                "moneyness": round(moneyness, 3), "iv": round(vol * 100, 1),
                "call_price": round(r.price, 2), "delta": round(r.delta, 4),
            })
