...
async def market_status(request: Request, current_user: dict = Depends(get_current_user)):
    ...
    if service is None and yf is not None:
        return {
            "connected": True,
            "healthy": True,
            "message": "Yahoo Finance fallback active",
                ...
                if bars and len(bars) >= 5:
                    ...
                    if len(closes) >= 5:
                        ...
                        return {
                            "regime": regime,
                            "confidence": 0.7,
                            "vol_percentile": round(vol * 100, 2),
        ...
        try:
            ...
            for sym in symbols[:10]:
                ...
                if bars and len(bars) >= 2:
                    last_bar = bars[-1]
                    prev_bar = bars[-2]
                    close = getattr(last_bar, "close", None) or (
                        last_bar.get("close") if isinstance(last_bar, dict) else 0
                    )
                    prev_close = getattr(prev_bar, "close", None) or (
                        prev_bar.get("close") if isinstance(prev_bar, dict) else 0
                    )
