...
class BrokerReconciliator:
    ...
    def _normalize_positions(self, positions: List[Any]) -> Dict[str, Dict[str, Any]]:
        ...
        normalized = {}
        for p in positions:
            symbol = getattr(p, "symbol", None) or p.get("symbol", "") if isinstance(p, dict) else getattr(p, "symbol", "")
            ...
            if key in normalized:
                ...
                normalized[key] = {"symbol": symbol, "exchange": exchange, "side": side, "qty": qty, "avg_price": avg_price}
        return normalized
    ...
    async def reconcile(self) -> ReconciliationReport:
        ...
        try:
            ...
            return ReconciliationReport(
                timestamp=timestamp, local_position_count=0, broker_position_count=0,
                matched=0, status="critical", error=f"Failed to get local positions: {e}",
            )
        ...
        try:
            ...
            if hasattr(broker_raw, '__await__'):
                try:
                    ...
                    return ReconciliationReport(
                        timestamp=timestamp, local_position_count=len(local_raw), broker_position_count=0,
                        matched=0, status="critical", error="Broker position fetch timed out (30s)",
                    )
            ...
            return ReconciliationReport(
                timestamp=timestamp, local_position_count=len(local_raw), broker_position_count=0,
                matched=0, status="critical", error=f"Failed to get broker positions: {e}",
            )
        ...
        for key in all_keys:
            ...
            if local and not broker:
                ...
                discrepancies.append(PositionDiscrepancy(
                    ...
                    local_avg_price=local["avg_price"], broker_avg_price=0,
                ...
                discrepancies.append(PositionDiscrepancy(
                    symbol=broker["symbol"], exchange=broker["exchange"],
                    local_qty=0, broker_qty=broker["qty"],
                    local_avg_price=0, broker_avg_price=broker["avg_price"],
                ...
                if qty_diff > self._qty_tolerance:
                    ...
                    severity = "critical" if notional > self._critical_threshold else (
                        "high" if notional > self._alert_threshold else "medium"
                    )
                    discrepancies.append(PositionDiscrepancy(
                        symbol=local["symbol"], exchange=local["exchange"],
                        local_qty=local["qty"], broker_qty=broker["qty"],
                        local_avg_price=local["avg_price"], broker_avg_price=broker["avg_price"],
        ...
        if status == "critical":
            logger.critical(
                ...
                len(discrepancies), total_impact,
            )
    ...
    async def _periodic_loop(self, interval_hours: float):
        ...
        while True:
            try:
                ...
                logger.info("Periodic reconciliation completed: status=%s, discrepancies=%d",
                           report.status, len(report.discrepancies))
