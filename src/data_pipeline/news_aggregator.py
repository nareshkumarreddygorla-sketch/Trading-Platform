...
class NewsAggregator:
    ...
    async def fetch_news(self, symbols: list[str] | None = None, market: str = "india") -> list[NewsArticle]:
        now = time.time()
        if now - self._last_fetch < self._cache_ttl and self._cache:
            if symbols:
                return [
                    ...
                    if any(s.upper() in [x.upper() for x in a.symbols] for s in symbols) or not a.symbols
                ]
            return self._cache
        ...
        if symbols:
            return [
                a for a in unique if any(s.upper() in [x.upper() for x in a.symbols] for s in symbols) or not a.symbols
            ]
        return unique

    async def _fetch_newsapi(self, symbols: list[str] | None, market: str) -> list[NewsArticle]:
        import httpx
    ...
    def get_sentiment_summary(self, articles: list[NewsArticle] | None = None) -> NewsSentimentSummary:
        ...
        return NewsSentimentSummary(
            ...
            top_positive=[a.title for a in positive[:3]],
